...
QString CheckFormulaForHydrate(const QString& formula){

    if(InputHaseCorrectChars(formula,"[^H2O]+$")) {

        return "Гидраты не рассчитываются";
...
QString CheckDBInput(const QString& acid_limit_input, const QString& equivalent_limit){
    ...
    } else if (!InputHaseCorrectChars(acid_limit_input, "[^A-Za-z0-9]")){ // \w
...
QString CheckNormalConcentrationInput(const QString& formula_input, const QString& float_input1, const QString& float_input2){
